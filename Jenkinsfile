node{
    ...
    stage('git code checkout'){
        try{
            ...
            git 'https://github.com/pal1506/star-agile-insurance-palproject-git'
        }
        catch(Exception e){
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t pal1506/insure-me:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u pal1506 -p ${dockerHubPassword}"
        sh "${dockerCMD} push pal1506/insure-me:${tagName}"
